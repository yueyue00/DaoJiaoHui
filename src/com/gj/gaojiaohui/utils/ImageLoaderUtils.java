...
public class ImageLoaderUtils {
	...
	public static DisplayImageOptions initOptions() {
		...
		// 设置图片在下载期间显示的图片
				.showImageOnLoading(R.drawable.dianping_img_default_nor)
				// 设置图片Uri为空或是错误的时候显示的图片
				.showImageOnFail(R.drawable.dianping_img_default_nor)
				// 设置下载的图片是否缓存在内存中
				.cacheInMemory(true)
				// 设置下载的图片是否缓存在SD卡中
				.cacheOnDisk(true)
				// 设置图片的url为空的时候显示的图片
				.showImageForEmptyUri(R.drawable.dianping_img_default_nor)
				// --------------------------------------------------------------------
	...
	/**
	 * 初始化DisplayImageOptions 针对圆角图
	 * 
	 * @param default_pic_id
	 *            默认图片
	 * @param circleAngle
	 *            圆角的角度
	 * @return
	 */
	public static DisplayImageOptions initOptionsForCirclePic(int default_pic_id, int circleAngle) {
		DisplayImageOptions options = new DisplayImageOptions.Builder()
		// 设置图片在下载期间显示的图片
				.showImageOnLoading(default_pic_id)
				// 设置图片Uri为空或是错误的时候显示的图片
				.showImageOnFail(default_pic_id)
				// 设置下载的图片是否缓存在内存中
				.cacheInMemory(true)
				// 设置下载的图片是否缓存在SD卡中
				.cacheOnDisk(true)
				// 设置图片的url为空的时候显示的图片
				.showImageForEmptyUri(default_pic_id)
				// --------------------------------------------------------------------
				// 如果您只想简单使用ImageLoader这块也可以不用配置
				// 是否考虑JPEG图像EXIF参数（旋转，翻转）
				.considerExifParams(true)
				...
				.resetViewBeforeLoading(true)
				// 是否设置为圆角，弧度为多少
				.displayer(new RoundedBitmapDisplayer(circleAngle))//ImageLoader 自带设置图片圆角
//				.displayer(new CircleBitmapDisplayer(circleAngle))// 设置图片变成圆形
																	// -22是圆角弧度　可以正数也可负数
																	// 默认是0
																	// 就是最圆的
				// 是否图片加载好后渐入的动画时间
	...
	/**
	 * 初始化DisplayImageOptions 专用于设置不同的默认图
	 * 
	 * @param default_pic_id
	 *            要设置的默认图
	 * @return
	 */
	public static DisplayImageOptions initOptionsToSetDefaultPic(int default_pic_id) {
		DisplayImageOptions options = new DisplayImageOptions.Builder()
		// 设置图片在下载期间显示的图片
				.showImageOnLoading(default_pic_id)
				// 设置图片Uri为空或是错误的时候显示的图片
