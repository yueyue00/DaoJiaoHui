...
/**
 * Created by linjizong on 15/6/12. 本地相册
 */
public class LocalAlbum extends GaoJiaoHuiBaseActivity {
	// 应用是否销毁标志
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			...
			case ImageUtils.REQUEST_CODE_GETIMAGE_BYCAMERA:
				...
				if (StringUtils.isNull(cameraPath)) {
	...
	public class FolderAdapter extends BaseAdapter {
		...
		public View getView(int i, View convertView, ViewGroup viewGroup) {
			...
			if (files.size() > 0) {
				ImageLoader.getInstance().displayImage(files.get(0).getThumbnailUri(), new ImageViewAware(viewHolder.imageView), options, null, null,
						files.get(0).getOrientation());
			}
			return convertView;
		}
